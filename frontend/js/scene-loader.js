// Adapted from powerOFMAX/fly-parking-lab's public/nmf/shared/scene.js
// (Apache License 2.0, see /THIRD_PARTY_NOTICES.md). Modified: the relative
// import path to the vendored MuJoCo build was changed to match this
// project's directory layout; docs-site theming (setupTheme) and the game/
// viewer-specific helpers (pool, makeStepper, makeStatsMeter) were dropped
// as unused; makeFailOverlay was kept but simplified (single call site,
// no app-name/tag parameters).
...
// One-call load sequence: start the MuJoCo runtime, fetch the MJCF + its meta,
// write them into MuJoCo's FS, compile, and return a reset + forwarded MjData.
// `onStage(text)` (optional) drives a loading overlay.
...
// Show a load error in the page's #overlay. Returns the `fail(msg, err)`
// handler.
export function makeFailOverlay(overlayEl) {
  return (msg, err) => {
    console.error(err || msg);
    overlayEl.classList.remove('hidden');
    overlayEl.innerHTML = `<div class="err"><strong>Could not load the scene.</strong><br>${msg}${err ? `<br><br><code>${String(err)}</code>` : ''}</div>`;
  };
}
